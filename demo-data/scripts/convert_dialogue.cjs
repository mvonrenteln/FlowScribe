...
function parseSegmentsFromDialog(text){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    const line = raw.trim();
    if(line === '') continue;
    if(isSectionDivider(line)) { section++; continue; }
...
function run(){
  ...
  if(fs.existsSync(outputPath)){
    ...
    try{ fs.copyFileSync(outputPath, bak, fs.constants.COPYFILE_EXCL); console.log('Backup created:', bak); }catch(e){ /* ignore if exists */ }
  }
