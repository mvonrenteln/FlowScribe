...
describe('segmentMerge utils', () => {
  const segA: MergeAnalysisSegment = { id: 'seg-1', speaker: 'A', start: 0, end: 1.2, text: 'Hello world' } as any;
  const segB: MergeAnalysisSegment = { id: 'seg-2', speaker: 'A', start: 1.3, end: 2.5, text: 'and welcome' } as any;
  ...
  it('sentence heuristics', () => {
    ...
    expect(detectIncorrectSentenceBreak({ text: 'So this is fine.' }, { text: 'and continues' })).toBe(true);
    expect(detectIncorrectSentenceBreak({ text: 'This ends' }, { text: 'Now a new sentence.' })).toBe(true);
  ...
  it('processSuggestion builds suggestion correctly', () => {
    ...
    const map = new Map([['seg-1', segA], ['seg-2', segB]]);
