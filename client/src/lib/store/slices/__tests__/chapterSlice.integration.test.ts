...
import { createHistorySlice, addToHistory } from '../historySlice';
import { initialChapterState } from '../../types'; // Assuming initial state or similar exists
import type { ChapterPrompt } from '@/lib/ai/features/chapterOperations/types';

// Mock executeFeature
const mockExecuteFeature = vi.hoisted(() =>
    vi.fn(async (featureId, variables) => {
...
const createMockStore = () => {
    ...
    let state: Partial<TranscriptStore> = {
        ...
        selectSegmentsInChapter: (id) => segments, // simplified
    };
