...
describe("aiRevisionSelectionSlice persistence", () => {
  ...
  it("loads initial selection from localStorage", () => {
    ...
    const scopedCreator = createAiRevisionSelectionSlice as unknown as StateCreator<
      Partial<TranscriptStore>,
      [],
      [],
      AIRevisionSlice
    >;
    const useTestStore = create<Partial<TranscriptStore>>((set) => ({ ...scopedCreator(set) }));
  ...
  it("writes selection to localStorage when set", () => {
    ...
    expect(useTestStore.getState().aiRevisionLastSelection).toEqual({
      providerId: "prov-2",
      model: "m-2",
    });
  });
