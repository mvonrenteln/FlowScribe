...
export function extractJsonArray(raw: string): OllamaResponseItem[] {
    ...
    const objectPattern = /\{\s*"id"\s*:\s*(\d+)\s*,\s*"tag"\s*:\s*"([^"]+)"(?:\s*,\s*"confidence"\s*:\s*([\d.]+))?(?:\s*,\s*"reason"\s*:\s*"([^"]*)")?\s*\}/gi;
...
function formatSegmentsForPrompt(batch: BatchSegment[]): string {
    return batch
        .map((s) => `ID ${s.simpleId}: [${s.speaker}] "${s.text}"`)
        .join("\n");
}
...
export async function* analyzeSegmentsBatched(
    ...
    // Filter segments
    const filteredSegments = filterSegmentsForAnalysis(
        segments,
        selectedSpeakers,
        excludeConfirmed,
    );
    ...
    const userPromptTemplate =
        activeTemplate?.userPromptTemplate ?? DEFAULT_USER_PROMPT_TEMPLATE;
