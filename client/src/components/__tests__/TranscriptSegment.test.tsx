...
describe("TranscriptSegment", () => {
  ...
  it("shows replace preview when the current match starts mid-word", () => {
    const onReplaceCurrent = vi.fn();
    const englishSegment: Segment = {
      ...segment,
      text: "Hello world",
    ...
    render(
      <TranscriptSegment
        ...
        searchQuery="ello"
        replaceQuery="ELLO"
        currentMatch={{ segmentId: "seg-1", startIndex: 1, endIndex: 5, text: "ello" }}
        onReplaceCurrent={onReplaceCurrent}
      />,
    );

    expect(screen.getAllByText("Replace Preview")).toHaveLength(1);
    expect(screen.getByText("ello", { selector: "code" })).toBeInTheDocument();
    expect(screen.getByText("ELLO")).toBeInTheDocument();
  });
