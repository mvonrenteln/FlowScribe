...
| **B — Permission needed** | `backupConfig.enabled && backupState.status !== "enabled"` | "Re-authorize & backup" → calls `FileSystemProvider.enable()` then `backupNow` |
...
- `client/src/components/backup/__tests__/DirtyUnloadBanner.test.tsx` — 13 tests

### Modified files
- `client/src/lib/backup/BackupScheduler.ts` — uses `setDirtyUnloadFlag()`, stores handler ref, cleans up in `stop()`
...
3. **No coordination with RestoreBanner:** Both banners render independently. The scenario where both appear simultaneously is extremely unlikely (requires dirty unload + restorable snapshot from a different session).
